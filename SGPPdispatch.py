...
Program Startup:
	- All startup sequence and overhead is implemented in SGPPinternal.
	- SGPPinternal also defines all of the global variables that can be borrowed by other modules (namely this one,
	and SGPPanalysis)
	- Importing SGPPinternal calls SGPPinternal.__init_sequence(), and reads all the inputs (config, PPdb,
	and fundamentals)
	- SGPPinternal is imported by this module


Author:         David Roman Kayanan
...
# IMPORTS
# ----------------------------------------------------- Project modules
import SGPPinternal as __SGPP
import PowerPlants as pp
get_plantclass = pp.get_plantclass
import NEMS as mkt

# Import globals from SGPPinternal into the work namespace
# Power plants
PPdb = __SGPP.PPdb
GenFleet = pp.GenUnit.get_fleet()  # GenFleet is just introduced here, and is not used in any of the functions
GenCos   = __SGPP.GenCos

# Input data
dFundamentals   = __SGPP.dFundamentals
dGenParameters  = __SGPP.dGenParameters
dParameters     = __SGPP.dParameters

# The rest
dStats = __SGPP.dStats
Scenarios   = __SGPP.Scenarios
PPdispatchError = __SGPP.PPdispatchError
# todo - will probably remove the PPdispatchError construct
if PPdispatchError is None:
	from SGPPanalysis import *
...
def set_period(t_from=None, t_to=None):
	...
	if t_from is None:
		t_from = __SGPP.time['common period'][0]
	if t_to is None:
		t_to = __SGPP.time['common period'][1]
	...
	if not(__SGPP.time['common period'][0] <= t_from <= __SGPP.time['common period'][1]):
		t_common_str = [dt.strftime("%Y %b %d") for dt in __SGPP.time['common period']]
		raise ValueError("Pls. pass a datetime.date within the common period [{}, {}].".format(*t_common_str), t_from)

	if not(__SGPP.time['common period'][0] <= t_to <= __SGPP.time['common period'][1]):
		t_common_str = [dt.strftime("%Y %b %d") for dt in __SGPP.time['common period']]
		raise ValueError("Pls. pass a datetime.date within the common period [{}, {}].".format(*t_common_str), t_to)

	# ----------------------------------------------------------------------------------- 2) Set __SGPP.time parameters
	__SGPP.time.update({
		'simulation period':    [t_from, t_to],
		'D_index':              pd.period_range(t_from, t_to, freq='D'),
		'DxP_index':            pd.period_range(t_from,
		                                        pd.Timestamp(t_to)+pd.Timedelta('1D')-__SGPP.time['periods duration'],
		                                        freq=__SGPP.time['DxP_freq']),
	})


	# ----------------------------------------------------------------------------------- 3) Done - log and report
	t_simul_str = [dt.strftime("%Y %b %d") for dt in __SGPP.time['simulation period']]
	N_days = len(__SGPP.time['D_index'])
...
def get_forex_D(day):
	...
	return mkt.get_forex_D(day,
       ...
       Scenarios=__SGPP.Scenarios,
       mkt_currency=__SGPP.config['market settings']['currency'])
...
def solve(simulation_name=None, runchecks=False, upon_exception='continue', get_tracking=False, daily_progress=False):
	...
	*If these are unnecessary, then implement a function that does nothing.
	...
	"""
	if __SGPP.time['simulation period'] == [None, None]:
	...
	report_str = "[{}] Market simulation launched for [{} to {}]\n\n{}\n".format(
		__SGPP.log_time(), *t_simul_str, Scenarios_str)
	print(report_str)
...
def get_rand_D():
	D0, DN = __SGPP.time['common period']
