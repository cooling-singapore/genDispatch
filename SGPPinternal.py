"""Underlying function for SGPPdispatch

DEV NOTES:
	The only changed done in refactoring the code is to make the PPdispatchError a local variable in __init_sequence(
	) and __check_PPdispatchError(), because PPdispatchError is a global variable in SGPPdispatch.
...
def reinit_fleet():
	...
	Calls the necessary routines from SGPPinternal subinit 4 and 5. Updates the GenFleet module name. This routine
	changes GenFleet only (all other work space names are untouched).
	"""
	# ------------------------------------------------------------------------ subinit4 step 1) Init fleet
	# Renews the global name GenFleet, but earlier imports of SGPPinternal would still have the previous fleet.
	# Thus, the modules that borrow from SGPPinternal should call pp.GenUnit.get_fleet()
	global GenFleet
	pp.GenUnit.init_fleet()
	GenFleet = pp.GenUnit.get_fleet()
