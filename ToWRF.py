...
print('Initializing SGPPdispatch\n')
import sys
...
# Paths
Path_toWRF = 'C:\\Users\\dkayanan\\main\\1 SG Plant Dispatch Model\\To WRF'
Path_InputFileStr = path.join(Path_toWRF, 'Input file structures')
...
def thermal_analysis(GenFleet, dParameters):
	...
	This is need only once, and is done after initializaing SGPPdispatch
	"""
...
def read_results(GenFleet):
	...
	print('\n')
	fp = path.join('C:\\Users\\dkayanan\\main\\1 SG Plant Dispatch Model\\Implementation\\Results', fname)
...
def calc_heatstreams(GenFleet, Results, by='outlet', latentheatfactors=None):
	...
	PARAMETERS:
		GenFleet        as in SGPPdispatch
		Results         results directory as in SGPPdispatch
...
def prep_WRF_inputs(GenFleet, day, WRF_cell_area=300*300, scenario=None, unit='W/m^2', write_to_disk=False,
	...
	PARAMETERS:
		...
		day                 The day ('YYYY MMM DD') of the 24-h profile (i.e. a day in the set period in SGPPdispatch).
		...
		scenario            (Optional; required if writing to disk) Name of the scenario. The files would be written in
							the directory "Input_<scenario>" in the Path_toWRF directory (will create a new directory if
							it doesn't exist).
		...
		PPcells_only        (Optional; defaults to False). If True, then only the WRF grid cells with power plants
							...
							introduced when studying the varaition of a 24-h profile.


	"""
	# --------------------------------------------------------------------------- 1) Initialize input files (pickled)
	WRF_SH = pd.read_pickle(path.join(Path_InputFileStr, 'WRF input - sensible heat, air.pkl'))
	WRF_LH = pd.read_pickle(path.join(Path_InputFileStr, 'WRF input - latent heat, air.pkl'))
	WRF_Sea = pd.read_pickle(path.join(Path_InputFileStr, 'WRF input - seawater.pkl'))

	# Get the cols of the 24-h profile
	SH_24h_cols = WRF_SH.columns[2:]
	LH_24h_cols = WRF_LH.columns[2:]
	Sea_24h_cols = WRF_Sea.columns[2:]

	# just make sure that the hourly profiles are zero
	WRF_SH.loc[:, SH_24h_cols] = 0.0
	WRF_LH.loc[:, LH_24h_cols] = 0.0
	WRF_Sea.loc[:, Sea_24h_cols] = 0.0

	# --------------------------------------------------------------------------- 2) Read the PP->WRF grid mapping file
	PPlocs = pd.read_pickle(path.join(Path_toWRF, 'Final PP mapping to WRF.pkl'))
	...
	if unit == 'W/m^2':
		...
		ValueError('Unexpected unit: {}'.format(unit))
	...
	if unit == 'MW':
		WRF_SH.columns = ['Longtiude', 'Latitude'] + list('Sensible_heat_(MW)_Hour{}'.format(hr) for hr in range(24))
		WRF_LH.columns = ['Longtiude', 'Latitude'] + list('Latent_heat_(MW)_Hour{}'.format(hr) for hr in range(24))
		WRF_Sea.columns = ['Longtiude', 'Latitude'] + list('Heat_into_sea_(MW)_Hour{}'.format(hr) for hr in range(24))


	# --------------------------------------------------------------------------- 6) Write to disk
	if write_to_disk:
		dir_name = path.join(Path_toWRF, 'Input_{}'.format(scenario))

		if not path.isdir(dir_name):
			mkdir(dir_name)

		WRF_SH.to_csv(path.join(dir_name, '{} {}_sensible_air.csv'.format(scenario, day)), index=True, header=True)
		WRF_LH.to_csv(path.join(dir_name, '{} {}_latent_air.csv'.format(scenario, day)), index=True, header=True)
		WRF_Sea.to_csv(path.join(dir_name, '{} {}_seawater.csv'.format(scenario, day)), index=True, header=True)
		print('Files written to: {}'.format(dir_name))
...
def analyze_variation(GenFleet):
	...
	for day in range(1, 31):
		...
		WRF_SH.columns = ['Longtiude', 'Latitude', ] + list_24h
		WRF_LH.columns = ['Longtiude', 'Latitude', ] + list_24h
		WRF_Sea.columns = ['Longtiude', 'Latitude', ] + list_24h
