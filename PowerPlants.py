...
class GenUnit():
	...
	def set_PPdb(cls, filepath, readprms):
		...
		"""
		# ----------------------------------------------------------------------- 0) Declarations
		master_cols = {'Power Plant', 'Owner/Operator', 'Main Power Producer', 'Vested (2004)', 'Vested (LNG)',
	...
	def init_fleet(cls, rowIDs='all'):
		"""Convenience function to initialize the units in the power plant database. Pass an iterable of indices of
	...
	def __init__(self, rowID):
		...
		self.is_ = {
			...
			'Cogen'        : 'cogen' in params_row.at['Technology'].lower(),
			'WtE'          : params_row.at['Technology'] == 'WtE ST',
		}
		...
		# The stats random vars are initialized by SGPPdis_internal.subinit5_preprocess()
	...
	def set_bidquantities(self, mode, minbid_size=None, min_Nbids=None, max_Nbids=None, bid_precision=None,
		...
		if mode not in (1,):
			...
			if max_Nbids * minbid_size > GenCap - Pmin:
				warnings.warn("Might not be able to satisfy the minbid_size and max_Nbids constraint. Pls. considering "
				              "lowering at least one of them.")
	...
	def calc_bids(self, main_mode='SRMC', sub_mode='shift all', **kwargs):
		...
		MODES:
			'SRMC'          The GenUnit bids at marginal cost. self.SRMarginalCosts_P is used and smoothened to have
							strictly-increasing bids. Only this mode is implemented in the first version of SGPP
							dispatch.
		...
		if main_mode == 'SRMC':
			...
			raise NotImplementedError('Only the SRMC-based and WtE bidding are implemented for now.')
...
def get_plantclass(PPclass, GenFleet, PPdb, synonyms=None, as_ids=False):
	...
	if fuelkey == '':
		...
		try:
			fuel_matches = set(_SerFuel[fuelkey])
		except TypeError:  # single match
			fuel_matches = {_SerFuel[fuelkey]}
		except KeyError:   # no matches; use fuzzy
			fuel_matches = set()
			fuel_key_matches = set(key for key, score in fuzzyLookUp(fuelkey, _SerFuel, matcheslim=None))
